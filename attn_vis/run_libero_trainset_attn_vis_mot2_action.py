#!/usr/bin/env python3
"""Visualize FiS MoT2 Libero trainset attention over Cosmos video tokens."""
...
from attn_vis.run_rlbench_trainset_attn_vis_mot2_action import (  # noqa: E402
    PrintAccelerator,
    action_summary,
    coerce_bool,
    log_message,
...
from scripts.train_mot2 import VLACotDataset  # noqa: E402
...
class LiberoTrainsetAttnVisConfig:
    ...
    model_path: str = "/mnt/amlfs-07/shared/physicalword/ckpt/pretrained/Janus-Pro-1B"
    action_model_path: str = "/mnt/amlfs-07/shared/physicalword/ckpt/pretrained/LaST0_Pretrain_AE_chunk16/tfmr"
    action_expert_path: str = "/mnt/amlfs-07/shared/physicalword/ckpt/pretrained/LaST0_Pretrain_AE_chunk16/tfmr"
    janus_init_mode: str = "janus_pro_ae_flow"
    janus_pro_model_path: str = "/mnt/amlfs-07/shared/physicalword/ckpt/pretrained/Janus-Pro-1B"
    cosmos_model_path: str = (
        "/mnt/amlfs-07/shared/physicalword/ckpt/pretrained/Cosmos-Predict2.5-2B/base/pre-trained/"
    ...
    data_path: str = (
        "/mnt/amlfs-07/shared/physicalword/data/libero_training_data_last05_lastest/"
    ...
    wan21_vae_path: str = "/mnt/amlfs-07/shared/physicalword/ckpt/pretrained/wan2.1_vae/original/Wan2.1_VAE.pth"
    ...
    action_detach_slow_prefix: int = 0
    action_detach_video_branch: int = 0
    action_detach_slow_video_branch: int = 0
...
class LiberoAttentionMapRecorder:
    def __init__(self, cfg: LiberoTrainsetAttnVisConfig, num_layers: int, slow_layer_count: int, fast_layer_count: int, log_file=None):
        self.cfg = cfg
        ...
        if self.slow_layer_count <= 0 or self.fast_layer_count <= 0:
            raise ValueError(f"Expected slow and fast MoT layers, got slow={self.slow_layer_count}, fast={self.fast_layer_count}.")
        if self.slow_layer_count + self.fast_layer_count != self.num_layers:
            raise ValueError(
                f"Layer counts must cover all wrappers: slow={self.slow_layer_count}, fast={self.fast_layer_count}, total={self.num_layers}."
            )
    ...
    def capture_slow_attention(
        ...
        *,
        ...
        if action_valid_mask is None:
            selected = torch.tensor([q_tokens.shape[1] - 2, q_tokens.shape[1] - 1], device=q_tokens.device, dtype=torch.long)
        else:
            valid = action_valid_mask.to(device=q_tokens.device, dtype=torch.bool)[0]
            positions = torch.nonzero(valid, as_tuple=False).flatten()
            if int(positions.numel()) < 2:
                raise ValueError("Need at least two valid slow-prefix tokens to visualize phyword_prev/phyword.")
    ...
    def _render_summary(
        ...
        *,
        ...
        height = col_label_h + len(list(layers)) * cell_h + max(0, len(list(layers)) - 1) * self.gap
        ...
        for col_idx, (query_label, slice_label, base_label) in enumerate(columns):
            ...
            short_query = query_label.replace("phyword_", "phy_")
    ...
    def _save_query(self) -> list[str]:
        slow_labels = ["phyword_prev", "phyword"]
        ...
        slow_img = self._render_summary(
            ...
            layers=slow_layers,
            title="slow final prefix: phyword_prev/phyword -> Cosmos slices",
        )
        action_img = self._render_summary(
            records=self.action_records,
            query_labels=action_labels,
            layers=action_layers,
            title=f"action denoise step {self._target_action_step()}: action00-action{self.action_chunk - 1:02d} -> Cosmos slices",
        )
...
def install_attention_map_recorder(model, cfg: LiberoTrainsetAttnVisConfig, log_file=None) -> Optional[LiberoAttentionMapRecorder]:
    ...
    for layer_idx, wrapper in enumerate(wrappers):
        if not hasattr(wrapper, "_libero_attn_vis_original_forward_action_prefix_and_cache"):
            wrapper._libero_attn_vis_original_forward_action_prefix_and_cache = wrapper.forward_action_prefix_and_cache
            wrapper.forward_action_prefix_and_cache = types.MethodType(
                make_patched_forward_action_prefix_and_cache(layer_idx),
                wrapper,
            )
        if not hasattr(wrapper, "_libero_attn_vis_original_forward_action_suffix_only"):
            wrapper._libero_attn_vis_original_forward_action_suffix_only = wrapper.forward_action_suffix_only
            wrapper.forward_action_suffix_only = types.MethodType(make_patched_forward_action_suffix_only(layer_idx), wrapper)

    model._libero_attention_map_recorder = recorder
    log_message(
        f"Installed Libero FiS MoT2 attention recorder: dir={output_dir}, layers={len(wrappers)} "
        f"(slow={slow_layer_count}, fast={fast_layer_count}), action_chunk={cfg.action_chunk}, "
        f"capture_mode={cfg.attention_visualization_capture_mode}, tile={recorder.tile_size}, gap={recorder.gap}",
        log_file,
    )
    return recorder
...
def main() -> None:
    ...
    with log_path.open("a", encoding="utf-8") as log_file:
        log_message(f"=== Libero trainset attention visualization start {time.strftime('%Y-%m-%d %H:%M:%S')} ===", log_file)
        ...
        log_message("=== Libero trainset attention visualization finished ===", log_file)
