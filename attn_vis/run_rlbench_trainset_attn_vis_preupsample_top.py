...
class AttentionMapRecorder:
    ...
    def capture_layer_attention(
        ...
        *,
        ...
        if latent_start < 0:
            raise ValueError(
                "Could not locate latent tokens: "
        ...
        query_tokens = torch.cat(
            [q_l[0, latent_start:latent_end, :, :], q_a[0, action_start:action_end, :, :]],
            dim=0,
        )
        expected_queries = self.action_chunk + self.total_latent_tokens
        if int(query_tokens.shape[0]) != expected_queries:
            raise ValueError(f"Expected {expected_queries} query maps, got {query_tokens.shape[0]}.")

        image_keys = k_l[0, image_positions, :, :]
        scale = 1.0 / math.sqrt(float(query_tokens.shape[-1]))
        scores = torch.einsum("qhd,khd->qhk", query_tokens.to(torch.float32), image_keys.to(torch.float32)) * scale
    ...
    def render_overview(self, layer_maps: dict[int, torch.Tensor]) -> Image.Image:
        row_label_w = 86
        col_label_h = 42
        num_cols = self.action_chunk + self.total_latent_tokens
        canvas = Image.new(
            "RGB",
        ...
        col_labels = self.latent_labels + [f"action_{idx:02d}" for idx in range(self.action_chunk)]
