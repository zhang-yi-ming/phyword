...
SHELL_LOG="${LOG_DIR}/test_rlbench_trainset_attn_vis_preupsample_top_shell_${EVAL_ARTIFACT_NAME}.log"
BASH_HPARAMS_FILE="${ATTENTION_VISUALIZATION_DIR}/test_rlbench_trainset_attn_vis_preupsample_top_hparams_${EVAL_ARTIFACT_NAME}.env"
...
set -x
trap 'rc=$?; echo "[ERROR] test_rlbench_trainset_attn_vis_preupsample_top.sh failed with exit code ${rc}"' ERR
...

{
  for key in \
    ...
    ATTN_VIS_TILE_SIZE ATTN_VIS_ALPHA ATTN_VIS_CAPTURE_MODE ATTN_VIS_TOP_RATIO CUDA_DEVICE SEED EMPTY_CACHE_EVERY
  do
    write_hparam "$key"
  done
} > "$BASH_HPARAMS_FILE"
...
python -u "${SCRIPT_DIR}/run_rlbench_trainset_attn_vis_preupsample_top.py" \
  --pretrained_checkpoint "$PRETRAINED_CHECKPOINT" \
