...
RUN_NAME="${RUN_NAME:-cosmos2B_janus1B_2expert_rlbench_keyframe_tokenlatent_hidden_wanvae_2loss_300ep_1chunk}"
...
EVAL_ARTIFACT_NAME="${EVAL_ARTIFACT_NAME:-${EVAL_TIMESTAMP}_${RUN_NAME}_global_token_attn}"
...
SHELL_LOG="${LOG_DIR}/test_rlbench_trainset_global_token_attn_heatmap_shell_${EVAL_ARTIFACT_NAME}.log"
BASH_HPARAMS_FILE="${ATTENTION_VISUALIZATION_DIR}/test_rlbench_trainset_global_token_attn_heatmap_hparams_${EVAL_ARTIFACT_NAME}.env"
...
USE_LATENT_HIDDEN_WAN_DOWNSAMPLE_SIM_LOSS="${USE_LATENT_HIDDEN_WAN_DOWNSAMPLE_SIM_LOSS:-1}"
...
ATTN_VIS_CELL_WIDTH="${ATTN_VIS_CELL_WIDTH:-112}"
ATTN_VIS_CELL_HEIGHT="${ATTN_VIS_CELL_HEIGHT:-28}"
...
set -x
trap 'rc=$?; echo "[ERROR] test_rlbench_trainset_global_token_attn_heatmap.sh failed with exit code ${rc}"' ERR
...
{
  for key in \
    SCRIPT_DIR LAST05_ROOT EXPERIMENTS_ROOT OUTPUT_ROOT_DIR RUN_NAME RUN_DIR CHECKPOINT_NAME PRETRAINED_CHECKPOINT \
    ...
    VIDEO_LOSS_WEIGHT LATENT_LOSS_WEIGHT USE_LATENT_HIDDEN_SIM_LOSS LATENT_HIDDEN_SIM_LOSS_MODE LATENT_HIDDEN_SIM_LOSS_WEIGHT \
    USE_LATENT_HIDDEN_WAN_DOWNSAMPLE_SIM_LOSS LATENT_HIDDEN_WAN_DOWNSAMPLE_SIM_LOSS_WEIGHT WAN21_VAE_PATH \
    ...
    ATTN_VIS_CAPTURE_MODE ATTN_VIS_CELL_WIDTH ATTN_VIS_CELL_HEIGHT CUDA_DEVICE SEED EMPTY_CACHE_EVERY
  do
    write_hparam "$key"
  done
} > "$BASH_HPARAMS_FILE"
...
python -u "${SCRIPT_DIR}/run_rlbench_trainset_global_token_attn_heatmap.py" \
  --pretrained_checkpoint "$PRETRAINED_CHECKPOINT" \
  ...
  --attention_visualization_cell_width "$ATTN_VIS_CELL_WIDTH" \
  --attention_visualization_cell_height "$ATTN_VIS_CELL_HEIGHT" \
