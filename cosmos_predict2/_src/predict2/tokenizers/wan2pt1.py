    ...
    if SMOKE or pretrained_path is None:
        ...
        if get_rank() == 0:
            if not INTERNAL:
                ...
                LOCAL_TOKENIZER_PATH = "/mnt/nas/zhangyiming/database/ckpt/pretrained/Cosmos-Predict2.5-2B/tokenizer.pth"

                if os.path.exists(LOCAL_TOKENIZER_PATH):
                    pretrained_path = LOCAL_TOKENIZER_PATH
                elif isinstance(pretrained_path, str) and os.path.exists(pretrained_path):
                    pass
                else:
