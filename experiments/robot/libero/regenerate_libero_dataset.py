...
def copy_hdf5_attrs(src, dst):
    for key, value in src.attrs.items():
        dst.attrs[key] = value


def update_env_resolution_attrs(attrs, image_resolution, control_freq):
...
def main(args):
    ...
    print(f"Using image_resolution={args.image_resolution}, control_freq={args.control_freq}")
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        copy_hdf5_attrs(orig_data, grp)
        update_env_resolution_attrs(grp.attrs, args.image_resolution, args.control_freq)
        ...
        for i in range(len(orig_data.keys())):
            ...
            if done:
                ...
                if "model_file" in demo_data.attrs:
                    ep_data_grp.attrs["model_file"] = demo_data.attrs["model_file"]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image_resolution", type=int, default=256,
                        help="Rendered camera height/width for regenerated observations.")
    parser.add_argument("--control_freq", type=int, default=10,
                        help="LIBERO environment control frequency used during replay.")
