...
def _run_attention_vis_smoke_cli() -> None:
    """Early smoke path that avoids importing LIBERO/draccus/model dependencies."""
    import argparse

    import numpy as smoke_np
    import torch as smoke_torch
    from PIL import Image as SmokeImage
    from PIL import ImageDraw as SmokeImageDraw
    from PIL import ImageFont as SmokeImageFont

    parser = argparse.ArgumentParser()
    parser.add_argument("--attention_vis_smoke", action="store_true")
    parser.add_argument("--output", default="/tmp/libero_attn_vis_smoke.png")
    args, _ = parser.parse_known_args()

    base = SmokeImage.new("RGB", (64, 64), (92, 104, 112))
    base_draw = SmokeImageDraw.Draw(base)
    base_draw.rectangle((12, 14, 52, 50), outline=(235, 236, 220), width=2)
    base_draw.line((0, 63, 63, 0), fill=(160, 180, 205), width=2)

    def colormap(values: smoke_np.ndarray) -> smoke_np.ndarray:
        values = smoke_np.clip(values, 0.0, 1.0)
        red = smoke_np.clip(1.5 * values - 0.2, 0.0, 1.0)
        green = smoke_np.clip(1.5 - 3.0 * smoke_np.abs(values - 0.5), 0.0, 1.0)
        blue = smoke_np.clip(1.2 - 1.5 * values, 0.0, 1.0)
        return (smoke_np.stack([red, green, blue], axis=-1) * 255.0).astype(smoke_np.uint8)

    def overlay(heat: smoke_np.ndarray) -> SmokeImage.Image:
        heat = smoke_np.asarray(heat, dtype=smoke_np.float32)
        heat = smoke_np.nan_to_num(heat, nan=0.0, posinf=0.0, neginf=0.0)
        heat_min = float(heat.min())
        heat_max = float(heat.max())
        if heat_max > heat_min:
            heat = (heat - heat_min) / (heat_max - heat_min)
        else:
            heat = smoke_np.zeros_like(heat, dtype=smoke_np.float32)
        try:
            resample = SmokeImage.Resampling.BILINEAR
        except AttributeError:
            resample = SmokeImage.BILINEAR
        heat_img = SmokeImage.fromarray(colormap(heat), mode="RGB").resize(base.size, resample=resample)
        return SmokeImage.blend(base, heat_img, 0.45)

    num_layers = 24
    action_chunk = 16
    num_cols = action_chunk + 1
    cell_w, cell_h = base.size
    row_label_w = 86
    col_label_h = 42
    canvas = SmokeImage.new(
        "RGB",
        (row_label_w + num_cols * cell_w, col_label_h + num_layers * cell_h),
        (18, 22, 28),
    )
    draw = SmokeImageDraw.Draw(canvas)
    try:
        font = SmokeImageFont.load_default()
    except Exception:
        font = None
    for col_idx, label in enumerate(["spatial"] + [f"action_{idx:02d}" for idx in range(action_chunk)]):
        draw.text((row_label_w + col_idx * cell_w + 5, 13), label, fill=(245, 248, 252), font=font)

    grid_side = 24
    x = smoke_torch.linspace(0.0, 1.0, grid_side * grid_side, dtype=smoke_torch.float32)
    for layer_idx in range(num_layers):
        y = col_label_h + layer_idx * cell_h
        draw.text((8, y + max(4, cell_h // 2 - 6)), f"layer_{layer_idx:02d}", fill=(245, 248, 252), font=font)
        for query_idx in range(num_cols):
            center = ((layer_idx * 17 + query_idx * 31) % (grid_side * grid_side)) / (grid_side * grid_side - 1)
            heat = smoke_torch.exp(-((x - center) ** 2) / 0.01)
            heat = (heat / heat.sum()).reshape(grid_side, grid_side).numpy()
            canvas.paste(overlay(heat), (row_label_w + query_idx * cell_w, y))
...
if "--attention_vis_smoke" in sys.argv:
    _run_attention_vis_smoke_cli()
    sys.exit(0)
...
class GenerateConfig:
    ...
    attention_visualization_dir: str = ""            # If set, save layer/query attention overview PNGs under this directory
...
def validate_config(cfg: GenerateConfig) -> None:
    ...
    cfg.attention_visualization_dir = str(getattr(cfg, "attention_visualization_dir", "") or "").strip()
    ...
    if cfg.attention_visualization_dir and int(cfg.action_chunk) != 16:
        raise ValueError(
            "Attention visualization is defined for exactly 16 action chunk tokens; "
            f"got action_chunk={cfg.action_chunk}."
        )
...
def log_resolved_inference_config(cfg: Any) -> None:
    ...
    print(f"  attention_visualization_enabled={int(bool(getattr(cfg, 'attention_visualization_dir', '')))}")
...
class AttentionMapRecorder:
    """Collect and render action-denoise bridge attention maps for one policy query."""

    def __init__(self, cfg: Any, num_layers: int):
    ...
    def finish_query(self, log_file=None) -> list[str]:
        if not self.active:
            return []
        try:
            saved_paths = self._save_query(log_file=log_file)
        finally:
            self.active = False
            self.image_token_mask = None
            self.base_image = None
    ...
    def capture_layer_attention(
        ...
        *,
        ...
        if not self.active:
            return
        if q_l.shape[0] != 1 or k_l.shape[0] != 1 or q_a.shape[0] != 1:
            raise ValueError(
                "Attention visualization currently expects batch size 1, "
                f"got q_l={tuple(q_l.shape)}, k_l={tuple(k_l.shape)}, q_a={tuple(q_a.shape)}."
            )
        if int(layer_idx) == 0:
            self.current_step += 1
        ...
        if int(query_tokens.shape[0]) != self.action_chunk + 1:
            raise ValueError(
                f"Expected {self.action_chunk + 1} query maps, got {query_tokens.shape[0]}."
            )

        image_keys = k_l[0, image_positions, :, :]
        scale = 1.0 / math.sqrt(float(query_tokens.shape[-1]))
        scores = torch.einsum(
            "qhd,khd->qhk",
            query_tokens.to(torch.float32),
            image_keys.to(torch.float32),
        ) * scale
    ...
    def _save_query(self, log_file=None) -> list[str]:
        ...
        for denoise_step, layer_maps in sorted(self.records.items()):
            ...
            path = os.path.join(query_dir, f"denoise_step_{denoise_step:02d}.png")
            overview.save(path)
            saved_paths.append(path)
        log_message(
            f"Saved {len(saved_paths)} attention overview PNGs to {query_dir}",
            log_file,
        )
        return saved_paths

    def _query_dir(self) -> str:
        task_label = "na" if self.metadata.get("task_id") is None else f"{self.metadata['task_id']:02d}"
        episode_label = "na" if self.metadata.get("episode_idx") is None else f"{self.metadata['episode_idx']:03d}"
        return os.path.join(
            self.output_dir,
            f"task_{task_label}_episode_{episode_label}"
            f"_query_{self.metadata['inference_idx']:03d}"
            f"_envstep_{self.metadata['step_idx']:04d}",
        )

    def render_overview(self, layer_maps: dict[int, torch.Tensor]) -> Image.Image:
        base = self.base_image.convert("RGB")
        cell_w, cell_h = base.size
        row_label_w = 86
        col_label_h = 42
        num_cols = self.action_chunk + 1
        canvas = Image.new(
            "RGB",
            (row_label_w + num_cols * cell_w, col_label_h + self.num_layers * cell_h),
            (18, 22, 28),
        )
        ...
        col_labels = ["spatial"] + [f"action_{idx:02d}" for idx in range(self.action_chunk)]
        for col_idx, label in enumerate(col_labels):
            x = row_label_w + col_idx * cell_w + 5
        ...
        for layer_idx in range(self.num_layers):
            ...
            if tuple(maps.shape) != (num_cols, self.expected_image_tokens):
                raise ValueError(
                    f"Layer {layer_idx} attention map shape must be "
                    f"{(num_cols, self.expected_image_tokens)}, got {tuple(maps.shape)}."
                )
            for col_idx in range(num_cols):
                heat = maps[col_idx].reshape(self.expected_grid_side, self.expected_grid_side).numpy()
                overlay = self._overlay_heatmap(base, heat)
                canvas.paste(overlay, (row_label_w + col_idx * cell_w, y))
        return canvas

    def _overlay_heatmap(self, base: Image.Image, heat: np.ndarray) -> Image.Image:
        ...
        color = self._colormap(heat)
        heat_img = Image.fromarray(color, mode="RGB")
        try:
            resample = Image.Resampling.BILINEAR
        except AttributeError:
            resample = Image.BILINEAR
        heat_img = heat_img.resize(base.size, resample=resample)
        return Image.blend(base, heat_img, self.alpha)
...
def install_attention_map_recorder(model, cfg: Any) -> Optional[AttentionMapRecorder]:
    ...
    if len(wrappers) != 24:
        raise ValueError(
            "Attention visualization is defined for exactly 24 MoT layers; "
            f"got {len(wrappers)} wrappers."
        )

    recorder = AttentionMapRecorder(cfg, num_layers=len(wrappers))
...
def run_attention_render_smoke(output_path: Optional[str] = None) -> str:
    """Render a synthetic 24x17 attention overview without loading LIBERO/model deps."""
    if output_path is None:
        output_path = "/tmp/libero_attn_vis_smoke.png"

    class _SmokeCfg:
        attention_visualization_dir = ""
        action_chunk = 16

    recorder = AttentionMapRecorder(_SmokeCfg(), num_layers=24)
    base = Image.new("RGB", (64, 64), (92, 104, 112))
    draw = ImageDraw.Draw(base)
    draw.rectangle((12, 14, 52, 50), outline=(235, 236, 220), width=2)
    draw.line((0, 63, 63, 0), fill=(160, 180, 205), width=2)
    recorder.base_image = base

    x = torch.linspace(0.0, 1.0, recorder.expected_image_tokens, dtype=torch.float32)
    layer_maps = {}
    for layer_idx in range(recorder.num_layers):
        maps = []
        for query_idx in range(recorder.action_chunk + 1):
            center = ((layer_idx * 17 + query_idx * 31) % recorder.expected_image_tokens) / (
                recorder.expected_image_tokens - 1
            )
            heat = torch.exp(-((x - center) ** 2) / 0.01)
            heat = heat / heat.sum()
            maps.append(heat)
        layer_maps[layer_idx] = torch.stack(maps, dim=0)

    overview = recorder.render_overview(layer_maps)
...
def run_episode(
    ...
    while t < max_steps + cfg.num_steps_wait:
        ...
        if len(action_queue) == 0:
            ...
            attention_recorder = getattr(model, "_attention_map_recorder", None)
            if attention_recorder is not None:
                attention_recorder.start_query(
                    image_token_mask=janus_images_seq_mask,
                    base_image=janus_primary_image,
                    task_id=task_id,
            ...
            if attention_recorder is not None:
                attention_recorder.finish_query(log_file=log_file)
...
def eval_libero(cfg: GenerateConfig) -> float:
    ...
    attention_recorder = install_attention_map_recorder(model, cfg)
    if attention_recorder is not None:
        log_message(
            f"Attention visualization enabled: {attention_recorder.output_dir}",
            log_file,
        )
