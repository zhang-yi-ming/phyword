...
class GenerateConfig:
    ...
    use_latent_hidden_sim_loss: bool = True           # Required here so the Wan hidden upsampler is built and checkpoint-loaded
    latent_hidden_sim_loss_mode: str = "wan_vae"      # This visualization only supports Wan-VAE-trained checkpoints
    wan21_vae_path: str = DEFAULT_WAN21_VAE_CKPT      # Wan2.1 VAE checkpoint used to decode hidden-derived latents
    ...
    save_latent_hidden_vae_images: bool = True        # Save hidden->upsampler->Wan VAE decoded image for each policy query
    latent_hidden_vae_image_dir: str = ""             # Directory for latent hidden VAE PNGs
...
def validate_config(cfg: GenerateConfig) -> None:
    ...
    cfg.save_latent_hidden_vae_images = coerce_bool(
        getattr(cfg, "save_latent_hidden_vae_images", True)
    )
    cfg.latent_hidden_vae_image_dir = str(
        getattr(cfg, "latent_hidden_vae_image_dir", "") or ""
    ).strip()
    cfg.use_latent_hidden_sim_loss = coerce_bool(
        getattr(cfg, "use_latent_hidden_sim_loss", True)
    )
    cfg.latent_hidden_sim_loss_mode = str(
        getattr(cfg, "latent_hidden_sim_loss_mode", "wan_vae") or ""
    ).lower()
    cfg.wan21_vae_path = str(getattr(cfg, "wan21_vae_path", "") or "").strip()
    ...
    if cfg.save_latent_hidden_vae_images:
        ...
        if not cfg.latent_hidden_vae_image_dir:
            artifact = cfg.eval_artifact_name or DATE_TIME
            cfg.latent_hidden_vae_image_dir = os.path.join(
                cfg.local_log_dir,
                "latent_hidden_vae_images",
                artifact,
            )
...
def model_load(cfg: Any):
    ...
    if bool(getattr(cfg, "save_latent_hidden_vae_images", False)):
        missing_upsampler_keys = [
            key for key in missing_keys if key.startswith("latent_hidden_wan_upsampler.")
        ]
    ...
    if bool(getattr(cfg, "save_latent_hidden_vae_images", False)):
        ...
        print(f"Latent hidden VAE image dir: {cfg.latent_hidden_vae_image_dir}")
...
def save_latent_hidden_vae_images(
    ...
    for batch_idx, frame in enumerate(frames):
        ...
        filename = (
            ...
            f"--batch={batch_idx:02d}--token_id={token_id}"
            f"--token={_sanitize_filename(token_label)}"
            f"--desc={processed_task_description}.png"
        )
