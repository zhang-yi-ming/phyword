#!/usr/bin/env python3
"""Evaluate beta token-latent Cosmos-Janus keyframe policy on RLBench via lift3d."""
...
class EvalConfig:
    ...
    use_latent_hidden_sim_loss: bool = True
    latent_hidden_sim_loss_mode: str = "wan_vae"
    wan21_vae_path: str = DEFAULT_WAN21_VAE_CKPT
    save_latent_hidden_vae_images: bool = True
    latent_hidden_vae_image_dir: str = ""
...
def _sanitize_filename(text: str) -> str:
    sanitized = str(text).lower().replace(" ", "_").replace("\n", "_").replace(".", "_").replace("/", "_")
    sanitized = sanitized.replace("\\", "_").replace(":", "_")
    return sanitized[:80]


def _token_label_from_id(processor, token_id: int) -> str:
...
def _draw_token_header(frame: np.ndarray, label: str) -> np.ndarray:
    ...
    header_height = 40
    canvas = Image.new("RGB", (pil_frame.width, pil_frame.height + header_height), (255, 255, 255))
    ...
    try:
        ...
        font = None
    max_chars = max(16, pil_frame.width // 7)
    text = label if len(label) <= max_chars else label[: max_chars - 3] + "..."
    draw.text((8, 12), text, fill=(0, 0, 0), font=font)
    return np.asarray(canvas, dtype=np.uint8)


def _decoded_wan_images_to_uint8(decoded: torch.Tensor) -> np.ndarray:
    images = decoded.detach().to(torch.float32).cpu()
    if images.ndim != 5:
        raise ValueError(f"Expected decoded Wan images [B, 3, T, H, W], got {tuple(images.shape)}")
    images = images[:, :, 0].permute(0, 2, 3, 1)
    images = ((images + 1.0) / 2.0).clamp(0.0, 1.0)
    return (images * 255.0).round().to(torch.uint8).contiguous().numpy()


def save_latent_hidden_vae_images(
    latent_visual_debug: Optional[dict[str, torch.Tensor]],
    cfg: EvalConfig,
    model,
    processor,
    task_name: str,
    ...
    if not getattr(cfg, "save_latent_hidden_vae_images", False):
        return []
    if latent_visual_debug is None:
        raise RuntimeError("Latent hidden VAE image saving was requested but no latent debug output was returned.")
    upsampler = getattr(model, "latent_hidden_wan_upsampler", None)
    decoder = model.__dict__.get("latent_hidden_wan_decoder", None)
    if upsampler is None or decoder is None:
        raise RuntimeError("Latent hidden VAE visualization requires a loaded Wan upsampler and decoder.")

    anchor_hidden = latent_visual_debug["latent_anchor_hidden"]
    token_ids = latent_visual_debug["latent_token_ids"].detach().cpu().view(-1).tolist()
    device = next(model.parameters()).device
    dtype = getattr(model, "dtype", torch.bfloat16)
    anchor_hidden = anchor_hidden.to(device=device, dtype=dtype)
    batch_size = int(anchor_hidden.shape[0])
    hidden_image = anchor_hidden.reshape(batch_size, int(model.janus_dim), 1, 1)

    with torch.inference_mode():
        pred_latent = upsampler(hidden_image)
        if pred_latent.ndim != 4 or pred_latent.shape[1] != Wan21VAEDecoder.latent_channels:
            raise ValueError(f"Unexpected Wan latent shape from upsampler: {tuple(pred_latent.shape)}")
        decoded = decoder.decode(pred_latent.to(torch.float32).unsqueeze(2))

    frames = _decoded_wan_images_to_uint8(decoded)
    save_dir = cfg.latent_hidden_vae_image_dir
    os.makedirs(save_dir, exist_ok=True)

    saved_paths = []
    for batch_idx, frame in enumerate(frames):
        token_id = int(token_ids[batch_idx])
        token_label = _token_label_from_id(processor, token_id)
        header_label = f"token: {token_label}  id: {token_id}"
        image = _draw_token_header(frame, header_label)
        filename = (
            f"task={_sanitize_filename(task_name)}"
            f"--episode={episode_idx:03d}--query={inference_idx:03d}"
            f"--step={step_idx:04d}--batch={batch_idx:02d}"
            f"--token_id={token_id}--token={_sanitize_filename(token_label)}.png"
        )
    ...
    for path in saved_paths:
        log_message(f"Saved latent hidden VAE image to {path}", log_file)
    return saved_paths
...
def maybe_infer_action_intermediate_size(cfg: EvalConfig, state_dict: dict[str, Any]) -> None:
...
def model_load(cfg: EvalConfig):
    ...
    processor = load_processor_for_checkpoint(cfg.model_path, base_dir)
    tokenizer = processor.tokenizer
    ...
    if bool(getattr(cfg, "save_latent_hidden_vae_images", False)):
        if not any(key.startswith("latent_hidden_wan_upsampler.") for key in state_dict):
            raise RuntimeError(
                "This visualization requires a checkpoint with trained "
                "latent_hidden_wan_upsampler weights."
            )
    ...
    if bool(getattr(cfg, "save_latent_hidden_vae_images", False)):
        missing_upsampler_keys = [
            key for key in missing if key.startswith("latent_hidden_wan_upsampler.")
        ]
        if missing_upsampler_keys:
            raise RuntimeError(
                "Checkpoint did not load all latent_hidden_wan_upsampler weights: "
                f"{missing_upsampler_keys[:8]}"
            )

    device = torch.device(f"cuda:{cfg.cuda}" if torch.cuda.is_available() else "cpu")
    model = model.to(torch.bfloat16).to(device).eval()
    if bool(getattr(cfg, "save_latent_hidden_vae_images", False)):
        if getattr(model, "latent_hidden_wan_upsampler", None) is None:
            raise RuntimeError("Model did not build latent_hidden_wan_upsampler.")
        model.__dict__["latent_hidden_wan_decoder"] = Wan21VAEDecoder(
            vae_pth=cfg.wan21_vae_path,
            dtype=torch.float32,
            device=device,
            freeze=True,
            normalized_latents=True,
        )
        logger.info("Latent hidden VAE image dir: %s", cfg.latent_hidden_vae_image_dir)
...
def predict_actions(
    ...
    janus_action_pixel_values = processor.image_processor([image], return_tensors="pt")["pixel_values"].unsqueeze(0).to(device).to(dtype)
    ...
    with torch.inference_mode():
        inference_outputs = model.forward_flow_joint_inference(
            ...
            return_latent_visual_debug=cfg.save_latent_hidden_vae_images,
        )
    latent_visual_debug = None
    if cfg.save_latent_hidden_vae_images:
        *inference_outputs, latent_visual_debug = inference_outputs
    pred_video, pred_action = inference_outputs[:2]
    save_latent_hidden_vae_images(
        latent_visual_debug=latent_visual_debug,
        cfg=cfg,
        model=model,
        processor=processor,
        task_name=task_name,
    ...
    for episode in range(cfg.num_episodes):
        ...
        for step in range(cfg.max_steps):
            ...
            if len(action_queue) == 0:
                ...
                actions, pred_video, action_debug = predict_actions(
                    cfg,
                    model,
                    processor,
                    action_tokenizer,
...
def parse_args() -> EvalConfig:
    ...
    for name in ("cosmos_self_only_bridge", "decosmos", "use_value_prediction", "use_action_value_prediction",
                 ...
                 "use_latent_hidden_sim_loss", "save_latent_hidden_vae_images",
    ...
    cfg.latent_hidden_sim_loss_mode = str(cfg.latent_hidden_sim_loss_mode or "").lower()
    cfg.wan21_vae_path = str(cfg.wan21_vae_path or "").strip()
    cfg.latent_hidden_vae_image_dir = str(cfg.latent_hidden_vae_image_dir or "").strip()
    if cfg.save_latent_hidden_vae_images:
        if not cfg.use_latent_hidden_sim_loss or cfg.latent_hidden_sim_loss_mode != "wan_vae":
            raise ValueError(
                "Latent hidden VAE visualization requires "
                "use_latent_hidden_sim_loss=true and latent_hidden_sim_loss_mode=wan_vae."
            )
        if not cfg.wan21_vae_path:
            raise ValueError("Latent hidden VAE visualization requires --wan21_vae_path.")
        if not os.path.exists(cfg.wan21_vae_path):
            raise FileNotFoundError(f"Wan2.1 VAE checkpoint not found: {cfg.wan21_vae_path}")
        if not cfg.latent_hidden_vae_image_dir:
            cfg.latent_hidden_vae_image_dir = os.path.join(cfg.result_dir, "latent_hidden_vae_images")
