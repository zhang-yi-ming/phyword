...
if [[ "${DISABLE_EVAL_VIDEO_SAVE}" =~ ^(1|true|TRUE|yes|YES)$ ]]; then
  ...
  ROLLOUT_VIDEO_DIR=""
  VALUE_VIS_DIR=""
  DISABLE_ROLLOUT_VIDEO="${DISABLE_ROLLOUT_VIDEO:-true}"
else
...
SEED="${SEED:-0}"
NUM_TRIALS_PER_TASK="${NUM_TRIALS_PER_TASK:-${NUM_EPISODES:-50}}"
FPS=20
...

{
  for key in \
    ...
    DISABLE_EVAL_VIDEO_SAVE DISABLE_ROLLOUT_VIDEO TASK_IDS PRED_VIDEO_DIR ROLLOUT_VIDEO_DIR VALUE_VIS_DIR LOCAL_LOG_DIR RUN_ID_NOTE SPECIAL_TOKEN_VOCAB \
    ...
    TASK_SUITE_NAME VIDEO_H VIDEO_W VIDEO_FRAMES NUM_COND_INPUT_FRAMES ACTION_DIM ACTION_CHUNK CUDA_DEVICE EGL_DEVICE_ID MUJOCO_EGL_DEVICE_ID SEED NUM_TRIALS_PER_TASK FPS COSMOS_DENOISE_STEPS NUM_OPEN_LOOP_STEPS CONTROL_FREQ ACTION_REPEAT
  do
    write_hparam "$key"
  done
} > "$BASH_HPARAMS_FILE"
...
python -u "$LAST05_ROOT/experiments/robot/libero/run_libero_eval_new.py" \
  ...
  --disable_rollout_video "$DISABLE_ROLLOUT_VIDEO" \
