...
DISABLE_EVAL_VIDEO_SAVE="${DISABLE_EVAL_VIDEO_SAVE:-1}"
if [[ "${DISABLE_EVAL_VIDEO_SAVE}" =~ ^(1|true|TRUE|yes|YES)$ ]]; then
  PRED_VIDEO_DIR=""
  ROLLOUT_VIDEO_DIR=""
  VALUE_VIS_DIR=""
else
...

{
  for key in \
    ...
    DISABLE_EVAL_VIDEO_SAVE PRED_VIDEO_DIR ROLLOUT_VIDEO_DIR VALUE_VIS_DIR LOCAL_LOG_DIR RUN_ID_NOTE \
    ...
    TOTAL_LATENT_TOKENS SPECIAL_TOKEN_VOCAB FUTURE_FRAME_STRIDE ROBOT_STATE STATE_PLACEHOLDER_TOKENS STATE_DIM STATE_ENCODING_MODE \
    ACTION_INTERMEDIATE_SIZE ACTION_USE_LATENT_PREFIX ACTION_INSERT_LAYER DECOSMOS COSMOS_SELF_ONLY_BRIDGE ACTION_SELF_CAUSAL_IN_BRIDGE \
    TASK_SUITE_NAME TASK_IDS VIDEO_H VIDEO_W VIDEO_FRAMES NUM_COND_INPUT_FRAMES ACTION_CHUNK NUM_TRIALS_PER_TASK CUDA_DEVICE SEED FPS COSMOS_DENOISE_STEPS NUM_OPEN_LOOP_STEPS CONTROL_FREQ ACTION_REPEAT
  do
    write_hparam "$key"
  done
} > "$BASH_HPARAMS_FILE"
...
if [[ -f "${COSMOS_CUDA_SHIM_ROOT}/cosmos_cuda.py" ]]; then
  export PYTHONPATH="${LAST05_ROOT}:${COSMOS_CUDA_SHIM_ROOT}:${LIBERO_ROOT}:${COSMOS_ROOT}:${PYTHONPATH:-}"
else
