...
LATENT_HIDDEN_VAE_IMAGE_DIR="${LATENT_HIDDEN_VAE_IMAGE_DIR:-${EXPERIMENTS_ROOT}/latent_hidden_vae_images/${EVAL_ARTIFACT_NAME}}"
...

{
  for key in \
    ...
    PRED_VIDEO_DIR ROLLOUT_VIDEO_DIR VALUE_VIS_DIR LATENT_HIDDEN_VAE_IMAGE_DIR LOCAL_LOG_DIR RUN_ID_NOTE \
    ...
    USE_LATENT_HIDDEN_SIM_LOSS LATENT_HIDDEN_SIM_LOSS_MODE WAN21_VAE_PATH SAVE_LATENT_HIDDEN_VAE_IMAGES \
...
echo "[INFO] predicted video dir: ${PRED_VIDEO_DIR:-disabled}"
...
# Launch LIBERO-Spatial evals with latent-hidden Wan VAE image visualization.
python -u "$LAST05_ROOT/experiments/robot/libero/run_libero_eval_new_latent_vae_vis.py" \
  --pretrained_checkpoint "$PRETRAINED_CHECKPOINT" \
