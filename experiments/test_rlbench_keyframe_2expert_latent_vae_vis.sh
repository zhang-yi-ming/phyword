...
RUN_NAME="${RUN_NAME:-cosmos2B_janus1B_2expert_rlbench_keyframe_tokenlatent_100ep_1chunk}"
...
LATENT_HIDDEN_VAE_IMAGE_DIR="${LATENT_HIDDEN_VAE_IMAGE_DIR:-${RESULT_DIR}/latent_hidden_vae_images}"
...
USE_LATENT_HIDDEN_SIM_LOSS="${USE_LATENT_HIDDEN_SIM_LOSS:-true}"
LATENT_HIDDEN_SIM_LOSS_MODE="${LATENT_HIDDEN_SIM_LOSS_MODE:-wan_vae}"
WAN21_VAE_PATH="${WAN21_VAE_PATH:-/mnt/nas/zhangyiming/database/ckpt/pretrained/wan2.1_vae/original/Wan2.1_VAE.pth}"
SAVE_LATENT_HIDDEN_VAE_IMAGES="${SAVE_LATENT_HIDDEN_VAE_IMAGES:-true}"
...

{
  for key in LAST05_ROOT EXPERIMENTS_ROOT OUTPUT_ROOT_DIR RUN_NAME RUN_DIR CHECKPOINT_NAME PRETRAINED_CHECKPOINT \
    ...
    TRAIN_PROMPT_JSON_PATH DEFAULT_TASK_NAMES TASK_IDS TASK_NAMES NUM_EPISODES MAX_KEYFRAME_CHUNKS MAX_STEPS CUDA_DEVICE SEED ACTION_DIM ACTION_CHUNK VIDEO_FRAMES NUM_COND_INPUT_FRAMES ENV_IMG_RES \
    TOTAL_LATENT_TOKENS IMG_LATENTS_PER_FUTURE STATE_LATENTS_PER_FUTURE NUM_FUTURE_FRAMES FUTURE_FRAME_STRIDE \
    ROBOT_STATE STATE_PLACEHOLDER_TOKENS STATE_DIM STATE_ENCODING_MODE ACTION_INTERMEDIATE_SIZE ACTION_USE_LATENT_PREFIX \
    DECOSMOS COSMOS_SELF_ONLY_BRIDGE BRIDGE_POS_SCHEME ACTION_SELF_CAUSAL_IN_BRIDGE ACTION_DENOISE_STEPS COSMOS_DENOISE_STEPS \
    FPS NUM_OPEN_LOOP_STEPS ACTION_REPEAT RESET_RETRIES RESET_RETRY_DELAY_SECONDS SAVE_KEYFRAME_VIDEO SAVE_ROLLOUT_IMAGES SAVE_COSMOS_VIDEOS USE_VALUE_PREDICTION USE_ACTION_VALUE_PREDICTION VALUE_TOKEN_MASK_VIDEO_TO_VALUE VALUE_TOKEN_MASK_NONVALUE_TO_VALUE \
    USE_LATENT_HIDDEN_SIM_LOSS LATENT_HIDDEN_SIM_LOSS_MODE WAN21_VAE_PATH SAVE_LATENT_HIDDEN_VAE_IMAGES LATENT_HIDDEN_VAE_IMAGE_DIR \
...
echo "[INFO] latent hidden VAE image dir: $LATENT_HIDDEN_VAE_IMAGE_DIR"

python -u "$LAST05_ROOT/experiments/robot/rlbench/run_rlbench_eval_keyframe_latent_vae_vis.py" \
  --pretrained_checkpoint "$PRETRAINED_CHECKPOINT" \
  ...
  --save_latent_hidden_vae_images "$SAVE_LATENT_HIDDEN_VAE_IMAGES" \
  --latent_hidden_vae_image_dir "$LATENT_HIDDEN_VAE_IMAGE_DIR" \
  --video_h 32 \
  --video_w 32
