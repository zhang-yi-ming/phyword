"""Test script for the 3-expert MoT model (Cosmos + Latent CoT + Action).

Inference pipeline: one-step cosmos → autoregressive CoT → multi-step action denoising.
"""
import os
import json
import torch
...
def load_inference_model(ckpt_dir, config):
    ...
    print(">>> 4. Building 3-Expert MoT Architecture & Loading Weights...")
    model = CosmosJanusMoT3Expert(cosmos_wrapper.net, cosmos_wrapper.tokenizer, janus_model, config)
    attach_cosmos_inference_runtime(model, cosmos_wrapper)

    ckpt_path = os.path.join(ckpt_dir, "cosmos_janus_mot3.pt")
...
def main():
    ...
    parser.add_argument('--num_latent_tokens', type=int, default=8, help='Number of latent CoT tokens to generate')
    parser.add_argument('--state_latents_per_future', type=int, default=0)
    parser.add_argument('--num_eval_chunks', type=int, default=4)
    parser.add_argument('--save_dir', type=str, default="./test_output_cot")
    ...
    print(f"\n>>> Starting Sequential Evaluation (3-Expert): {max_steps//stride} chunks, Stride={stride} frames.")
    ...
    for step_idx in range(0, max_steps, stride):
        ...
        if config.robot_state and state_gt is not None:
            norm_state = np.where(
                ...
                np.clip(2 * (state_gt - statistic['state_q01']) / (statistic['state_q99'] - statistic['state_q01'] + 1e-8) - 1.0, -1.0, 1.0),
                state_gt
            )
    ...
    print(f"Done! All assets saved to '{config.save_dir}'.")
