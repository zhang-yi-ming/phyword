"""
Training script for 3-Expert MoT: Cosmos (video) + Janus Latent CoT + Janus Action.

Key differences from train.py (2-expert):
  - Loads latent expert from vanilla Janus-Pro-1B (--model_path)
  - Loads action expert from a separately pretrained action-only checkpoint (--action_expert_path)
  - Copies action expert weights (non-suffixed) into _action-suffixed slots in the dual-expert architecture
  - Reads one- or two-token gtlatent/gtlatent2 labels from the dataset JSON
  - Forward returns 4 losses: total, video, action, latent CE

Usage:
  accelerate launch ... train_cot.py \
    --model_path /path/to/Janus-Pro-1B \
    --action_expert_path /path/to/action_pretrained_checkpoint ...
"""
...
import models.cosmos_janus_cot as cosmos_janus_cot_module
...
def load_causallm_with_loading_info(model_path: str, **kwargs):
    with suppress_transformers_loading_warnings():
    ...
    unloaded_keys = extract_unloaded_keys(loading_info)
    return model, loading_info, unloaded_keys
...
class VLACotDataset(Dataset):
    ...
    def _build_state_placeholder_text(self):
        ...
        return self.pad_token_text * self._state_placeholder_count()
    ...
    def __getitem__(self, index):
        ...
        if self.config.robot_state:
            ...
            if self._state_encoding_mode() == 'mlp':
                ...
                current_state_ids = torch.LongTensor(
                    self.tokenizer.encode(self.action_tokenizer(norm_state), add_special_tokens=False)
                )
                now_state = self._validate_current_state_ids(current_state_ids)
        ...
        # Keep Assistant as an open prefix. We do not add Assistant content through
        # the DeepSeek template because that would append sep2.
        prompt = user_prompt + "\n\n<|Assistant|>:"
        ...
        if 'video_action_path' not in sample:
            raise KeyError("Sample is missing required 'video_action_path' field.")
        action_frame_pil = self._load_frame_pil(sample['video_action_path'], frame_idx)
        ...
        if int(getattr(self.config, "use_latent_hidden_sim_loss", 0) or 0):
            ...
            if self.latent_hidden_sim_loss_mode == "wan_vae":
                ...
                latent_hidden_sim_pixel_values = self.processor.image_processor(
                    [future_frame_pil],
                    return_tensors="pt",
                )['pixel_values']
        ...
        item = {
            ...
            "janus_pixel_values": janus_inputs.pixel_values.squeeze(0),
        ...
        if (
            getattr(self.config, "use_value_prediction", 0)
            or getattr(self.config, "use_action_value_prediction", 0)
        ):
            item["value"] = self._compute_value_target(sample["video_path"], frame_idx)
        return item
    ...
    def collate_fn(self, batch):
        ...
        out = {
            ...
            "janus_action_pixel_values": torch.stack([x['janus_action_pixel_values'] for x in batch]),
        ...
        if (
            ...
            out["value"] = torch.stack([x["value"] for x in batch])
        return out
...
def train(args):
    ...
    # ----------------------------------------------------------------
    # Load latent expert from vanilla Janus-Pro-1B.
    # This provides the default (non-_action) LayerNorm/MLP components
    # that the LatentBridgeBlock will use, plus the vision encoder and aligner.
    # ----------------------------------------------------------------
    accelerator.print("Loading Janus-Pro-1B (latent expert base)...")
    ...
    accelerator.print(
        "Captured janus_model loading info "
        f"(missing={len(janus_model_loading_info.get('missing_keys', []))}, "
        f"unexpected={len(janus_model_loading_info.get('unexpected_keys', []))}, "
        f"mismatched={len(janus_model_loading_info.get('mismatched_keys', []))}, "
        f"unloaded_total={len(janus_model_unloaded_keys)})."
    )
    accelerator.print("janus_model1")
    #accelerator.print(janus_model)
    


    # ----------------------------------------------------------------
    # Load action expert from the action-only pretrained checkpoint.
    # This checkpoint has standard (non-suffixed) parameter names but
    # contains weights trained specifically for action prediction.
    # We copy them into the _action-suffixed slots of the dual-expert model.
    # ----------------------------------------------------------------
    accelerator.print("Loading action expert checkpoint...")
    janus_action, janus_action_loading_info, janus_action_unloaded_keys = load_causallm_with_loading_info(
    ...
    accelerator.print(
        "Captured janus_action loading info "
        f"(missing={len(janus_action_loading_info.get('missing_keys', []))}, "
        f"unexpected={len(janus_action_loading_info.get('unexpected_keys', []))}, "
        f"mismatched={len(janus_action_loading_info.get('mismatched_keys', []))}, "
        f"unloaded_total={len(janus_action_unloaded_keys)})."
    )
    #accelerator.print("janus_action2")
    #accelerator.print(janus_action)

    action_state = janus_action.state_dict()
    #accelerator.print("action_state1")
    
    #accelerator.print(action_state)
    n_copied = 0
    for name, param in janus_model.named_parameters():
        if '_action' in name:
            #accelerator.print("debug")
            # Map _action-suffixed name to the non-suffixed key in the action checkpoint
            base_name = name.replace('_action', '')
            if base_name in action_state:
                param.data.copy_(action_state[base_name])
                n_copied += 1
                accelerator.print(f"  Action expert: {name} <- {base_name}")
        elif any(k in name for k in ('x_embedder', 't_embedder', 'final_layer', 'state_embedder','embed_tokens')):
            # Flow matching components come from the action checkpoint
            if name in action_state:
                param.data.copy_(action_state[name])
                n_copied += 1
                accelerator.print(f"  Flow component: {name} <- action ckpt")

    

    accelerator.print(f"Copied {n_copied} parameter tensors from action expert checkpoint.")
    del janus_action, action_state
    gc.collect()
    ...
    # Build 3-Expert MoT
    # ----------------------------------------------------------------
    accelerator.print("Building 3-Expert Cosmos-Janus MoT VLA...")
    args.janus_image_start_id = processor.image_start_id
    args.janus_image_end_id = processor.image_end_id
    if args.janus_image_start_id is None:
        args.janus_image_start_id = processor.tokenizer.convert_tokens_to_ids("<begin_of_image>")
    if args.janus_image_end_id is None:
    ...
    accelerator.print(
        f"cosmos_self_only_bridge={int(bool(getattr(args, 'cosmos_self_only_bridge', 0)))}"
    )
    accelerator.print(
        f"train_embed_tokens={int(bool(getattr(args, 'train_embed_tokens', 0)))}"
    )
    accelerator.print(
        f"no_detach_latent_input={int(bool(getattr(args, 'no_detach_latent_input', 0)))}"
    )
    accelerator.print(f"state_encoding_mode={getattr(args, 'state_encoding_mode', 'token')}")
    accelerator.print(
        f"decosmos={int(bool(getattr(args, 'decosmos', 0)))}"
    )
    accelerator.print(
        f"cosmos_text_cache_enabled={int(bool(getattr(args, 'cosmos_text_cache_path', '')))}"
    )
    accelerator.print(
        f"action_use_latent_prefix={int(bool(getattr(args, 'action_use_latent_prefix', 0)))}"
    )
    accelerator.print(f"latent_token_mode={getattr(args, 'latent_token_mode', 'v')}")
    accelerator.print(f"latent_token_fields={','.join(getattr(args, 'latent_token_fields', ['gtlatent']))}")
    accelerator.print(f"total_latent_tokens={getattr(args, 'total_latent_tokens', 1)}")
    accelerator.print(
        f"use_history_trajectory_janus_image={int(bool(getattr(args, 'use_history_trajectory_janus_image', 0)))}"
    )
    accelerator.print(
        f"use_action_value_prediction={int(bool(getattr(args, 'use_action_value_prediction', 0)))}"
    )
    ...
    accelerator.print(f"value_loss_weight={getattr(args, 'value_loss_weight', 0.0)}")
    accelerator.print(f"action_value_loss_weight={getattr(args, 'action_value_loss_weight', 0.0)}")
    accelerator.print(
        f"value_token_mask_video_to_value={int(bool(getattr(args, 'value_token_mask_video_to_value', 0)))}"
    )
    accelerator.print(
        f"value_token_mask_nonvalue_to_value={int(bool(getattr(args, 'value_token_mask_nonvalue_to_value', 0)))}"
    )
    ...
    accelerator.print(f"cosmos_janus_cot_module={getattr(cosmos_janus_cot_module, '__file__', 'N/A')}")
    ...
    for name, param in model.named_parameters():
        ...
        if 'cosmos_dit' in name:
            if 'latent_bridge' in name or 'action_bridge' in name:
    ...
    world_size = dist.get_world_size() if dist.is_available() and dist.is_initialized() else 8
    ...
    for epoch in range(args.n_epochs):
        ...
        for batch in train_iter:
            with accelerator.accumulate(model):
                ...
                janus_action_pixel_values = batch['janus_action_pixel_values'].to(accelerator.device).to(torch.bfloat16)
                ...
                latent_hidden_sim_pixel_values = batch.get('latent_hidden_sim_pixel_values')
                if latent_hidden_sim_pixel_values is not None:
                    latent_hidden_sim_pixel_values = latent_hidden_sim_pixel_values.to(
                        accelerator.device,
                        non_blocking=True,
                    ).to(torch.bfloat16)
                ...
                value_targets = None
                if args.use_value_prediction or args.use_action_value_prediction:
                    value_targets = batch['value'].to(accelerator.device, non_blocking=True)
                ...
                with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                    loss, v_loss, a_loss, latent_ce_loss, aux_metrics = model(
                        ...
                        now_state=now_state,
                        fps=args.fps,
                        latent_gt_token_ids=latent_gt_token_ids,
                        latent_hidden_sim_pixel_values=latent_hidden_sim_pixel_values,
                        cosmos_text_embeddings=cosmos_text_embeddings,
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    log_payload = {
                        ...
                        'video_loss': v_loss,
                        'action_loss': a_loss,
                        'latent_ce_loss': latent_ce_loss,
                        'lr': lr_scheduler.get_last_lr()[1],
                    }
                    for key in (
                        "cosmos_value_loss",
                        "action_value_loss",
                        "latent_hidden_wan_vae_mse_loss",
                        "latent_hidden_wan_downsample_sim_loss",
                    ):
                        if key in aux_metrics:
                            log_payload[key] = aux_metrics[key]
                    if "latent_hidden_sim_loss" in aux_metrics:
                        log_payload["latent_hidden_sim_loss"] = aux_metrics["latent_hidden_sim_loss"]
                    wandb.log(log_payload, step=global_step)

                    postfix = {
                        "v": f"{v_loss:.4f}",
                        "a": f"{a_loss:.4f}",
                        "ce": f"{latent_ce_loss:.4f}",
                    }
                    if "cosmos_value_loss" in aux_metrics:
                        postfix["cval"] = f"{aux_metrics['cosmos_value_loss']:.4f}"
                    if "action_value_loss" in aux_metrics:
                        postfix["aval"] = f"{aux_metrics['action_value_loss']:.4f}"
                    if "latent_hidden_sim_loss" in aux_metrics:
                        postfix["hsim"] = f"{aux_metrics['latent_hidden_sim_loss']:.4f}"
                    if "latent_hidden_wan_vae_mse_loss" in aux_metrics:
                        postfix["wvae"] = f"{aux_metrics['latent_hidden_wan_vae_mse_loss']:.4f}"
                    if "latent_hidden_wan_downsample_sim_loss" in aux_metrics:
                        postfix["wdsim"] = f"{aux_metrics['latent_hidden_wan_downsample_sim_loss']:.4f}"
...
if __name__ == '__main__':
    ...
    # Base config
    parser.add_argument('--experiment_name', type=str, default='cosmos_janus_3expert')
    parser.add_argument('--run_name', type=str, default='run_1')
    parser.add_argument('--model_path', type=str, required=True,
                        help='Path to vanilla Janus-Pro-1B (provides latent expert + vision encoder)')
    ...
    args.bridge_pos_scheme = cosmos_janus_cot_module.normalize_bridge_pos_scheme(args.bridge_pos_scheme)
    ...
    if args.no_detach_latent_input not in (0, 1):
        raise ValueError("no_detach_latent_input must be 0 or 1.")
    if args.no_detach_latent_input and not args.train_embed_tokens:
        raise ValueError("no_detach_latent_input=1 requires train_embed_tokens=1 so token embeddings keep gradients.")
    if args.action_gt_latent_after_epoch < -1:
        raise ValueError(
            "action_gt_latent_after_epoch must be -1 or >= 0. "
            "Use -1 to keep the current legacy two-pass mode forever, or 0 to start "
            "single-forward GT latent action training from epoch 1."
        )
    ...
    if args.decosmos not in (0, 1):
        raise ValueError("decosmos must be 0 or 1.")
    if args.use_value_prediction not in (0, 1):
        raise ValueError("use_value_prediction must be 0 or 1.")
    if args.use_action_value_prediction not in (0, 1):
        raise ValueError("use_action_value_prediction must be 0 or 1.")
    ...
    if args.value_token_mask_video_to_value not in (0, 1):
        raise ValueError("value_token_mask_video_to_value must be 0 or 1.")
    if args.value_token_mask_nonvalue_to_value not in (0, 1):
        raise ValueError("value_token_mask_nonvalue_to_value must be 0 or 1.")
    if args.value_token_mask_nonvalue_to_value:
        args.value_token_mask_video_to_value = 1
    if args.use_value_prediction and args.decosmos:
    ...
    if args.action_value_loss_weight < 0:
        raise ValueError("action_value_loss_weight must be non-negative.")
