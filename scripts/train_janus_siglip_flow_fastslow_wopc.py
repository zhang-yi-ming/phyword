...
def train(args: argparse.Namespace) -> None:
    ...
    #     vision_params = get_param_count(model.vision_model)
    #     accelerator.print(f"Vision Encoder: {vision_params / 1e6:.2f}M ({vision_params / 1e9:.2f}B)")
    
    # # 2. Aligner / Projector (视觉-语言对齐层)
    # if hasattr(model, 'aligner'):
    #     aligner_params = get_param_count(model.aligner)
    #     accelerator.print(f"Aligner: {aligner_params / 1e6:.2f}M")

    # # 3. LLM Backbone (语言模型主体)
    # # Janus 通常将 LLM 放在 language_model 属性下
    # if hasattr(model, 'language_model'):
    #     llm_params = get_param_count(model.language_model)
    #     accelerator.print(f"LLM Backbone: {llm_params / 1e9:.2f}B")
        
    # # 4. Action/Diffusion Heads (如果你的模型有这些特定的头)
    # head_params = 0
    # for head_name in ['x_embedder', 't_embedder', 'final_layer', 'state_embedder']:
    #     if hasattr(model, head_name):
    #         p_count = get_param_count(getattr(model, head_name))
    #         head_params += p_count
    #         accelerator.print(f"Head ({head_name}): {p_count / 1e6:.2f}M")
    
    # # 5. Gen Vision Model (如果有图像生成部分的编码器)
    # if hasattr(model, 'gen_vision_model'):
    #     gen_vision_params = get_param_count(model.gen_vision_model)
    #     accelerator.print(f"Gen Vision Model: {gen_vision_params / 1e6:.2f}M")
