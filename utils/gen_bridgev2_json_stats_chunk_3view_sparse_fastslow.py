...
IMAGE_VIEWS_SAVE = ['image_primary'] 
IMAGE_VIEWS_SLOW = ['image_primary'] 
IMAGE_VIEWS_FAST = ['image_primary'] 
FUTURE_STEPS = 4
FAST_SLOW_RATIO = 4
LATENT_STRIDE = 5
        ...
        for task in task_lists:
            ...
            for file in os.listdir(f'{data_root}/{task}'):
                ...
                for i in range(episode_length):
                    ...
                    # --- Latent CoT Targets (修改部分) ---
                    for k in range(1, FUTURE_STEPS + 1):
                        # [修改] 使用 LATENT_STRIDE 计算目标索引
                        # 原来是: slow_idx + k
                        # 现在是: slow_idx + k * LATENT_STRIDE
...
data_root = "/media/liuzhuoyang/data/bridgev2/npy"
img_save_root = "/media/liuzhuoyang/LCoT_VLA_MOT/training_data/bridge_single_sparse_fastslow"
json_save_root = "/media/liuzhuoyang/LCoT_VLA_MOT/training_data/bridge_single_json"

jsonl_filename = f'{json_save_root}/bridgev2_view1_chunk4_fast4_sparse_fastslow_train.jsonl'
json_file = f'{json_save_root}/bridgev2_view1_chunk4_fast4_sparse_fastslow_train.json'
