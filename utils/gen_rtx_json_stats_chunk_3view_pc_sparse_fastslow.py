...
def create_padding_assets(save_dir, sample_step):
    ...
    if 'state' in sample_step:
        state_arr = np.array(sample_step['state']).reshape(-1)
        state_dim = state_arr.shape[0]
                ...
                for i in range(episode_length):
                    ...
                    for k in range(1, FUTURE_STEPS + 1):
                        ...
                        if tgt_idx < episode_length:
                            ...
                            state_val = np.nan_to_num(state_val, nan=0.0) 
                            state_val = state_val.reshape(-1).tolist()
...
def cal_stats(jsonl_filename):
    ...
    if np.isnan(actions).any():
        print(f"Warning: Found {np.isnan(actions).sum()} NaNs in Actions, replacing with 0.0")
        actions = np.nan_to_num(actions, nan=0.0)
        
    if np.isnan(states).any():
        print(f"Warning: Found {np.isnan(states).sum()} NaNs in States, replacing with 0.0")
...
data_root = "/media/mixiangju/Data/rtx_3view_1210_3D"
img_save_root = "/media/liuzhuoyang/LCoT_VLA_MOT/training_data/rtx_two_sparse_fastslow"
json_save_root = "/media/liuzhuoyang/LCoT_VLA_MOT/training_data/rtx_two_json"

jsonl_filename = f'{json_save_root}/rtx_mm_view2_chunk4_8_stride8_fast1_sparse_fastslow_train.jsonl'
json_file = f'{json_save_root}/rtx_mm_view2_chunk4_8_stride8_fast1_sparse_fastslow_train.json'
task_lists = [
  'rtx_1215'
]
